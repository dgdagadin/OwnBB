<?php

//Профиль пользователя
$ForumLang['UserProfileTitle'] = 'Профиль';
$ForumLang['UserProfileImportantFields'] = 'Поля, обязательные для заполнения';
$ForumLang['UserProfileOnline'] = 'в данный момент находится на форуме';
$ForumLang['UserProfileOffline'] = 'В данный момент нет на форуме';
$ForumLang['UserProfileLastVisit'] = 'Последний раз был на форуме';
$ForumLang['UserProfileEmpty'] = 'Не заполнено';
$ForumLang['UserProfileIn'] = 'в';
$ForumLang['UserProfileCommon'] = 'Общее';
$ForumLang['UserProfileActivity'] = 'Активность на форуме';
$ForumLang['UserProfileOnline'] = 'сейчас на форуме';
$ForumLang['UserProfileOffline'] = 'сейчас вне форума';
$ForumLang['UserProfileIn'] = 'в';
$ForumLang['UserProfileBirthLimit'] = 'в формате дд.мм.гггг';
$ForumLang['UserProfileLastAction'] = 'последний визит';
$ForumLang['UserProfilePersonals'] = 'Личные данные';
$ForumLang['UserProfileStatistics'] = 'Данные форума';
$ForumLang['UserProfileContacts'] = 'Контактные данные';
$ForumLang['UserProfileMailOpts'] = 'Дополнительные опции';
$ForumLang['UserProfileAvatarTitle'] = 'Аватар на форуме';
$ForumLang['UserProfileCurrentAvatar'] = 'Текущий аватар пользователя';
$ForumLang['UserProfileAvatar'] = 'Редактировать аватар';
$ForumLang['UserProfileMale'] = 'Мужской';
$ForumLang['UserProfileFemale'] = 'Женский';
$ForumLang['UserProfileLogin'] = 'Логин на форуме';
$ForumLang['UserProfileSlogan'] = 'Девиз';
$ForumLang['UserProfileSloganLimit'] = 'максимум {max} символов';
$ForumLang['UserProfilePassword'] = 'Новый пароль';
$ForumLang['UserPasswordLimit'] = '{min} - {max} символов';
$ForumLang['UserProfilePassRepeat'] = 'Повторите пароль';
$ForumLang['UserProfileBirth'] = 'Дата рождения';
$ForumLang['UserProfileSex'] = 'Пол';
$ForumLang['UserProfileCountry'] = 'Страна';
$ForumLang['UserProfileCountryLimit'] = '{min} - {max} символов';
$ForumLang['UserProfileWWW'] = 'Домашняя страница';
$ForumLang['UserProfileWWWLimit'] = '{min} - {max} символов';
$ForumLang['UserProfileCity'] = 'Город';
$ForumLang['UserProfileCityLimit'] = '{min} - {max} символов';
$ForumLang['UserProfileGroup'] = 'Группа на форуме';
$ForumLang['UserProfileStatus'] = 'Статус на форуме';
$ForumLang['UserProfileRegDate'] = 'Дата регистрации';
$ForumLang['UserProfileNumber'] = 'Номер пользователя';
$ForumLang['UserProfileThemes'] = 'Тем создано';
$ForumLang['UserProfilePosts'] = 'Постов создано';
$ForumLang['UserProfileMail'] = 'Электронная почта';
$ForumLang['UserProfileMailLimit'] = '{min} - {max} символов';
$ForumLang['UserProfileHidden'] = 'Скрывать';
$ForumLang['UserProfileAvatarDel'] = 'Удалить аватар';
$ForumLang['UserProfileAvatarNew'] = 'Загрузить новый аватар';
$ForumLang['UserProfileShowAvatarTitle'] = 'Аватар пользователя';
$ForumLang['UserProfileNoAvatar'] = 'Пользователь еще не загрузил аватар';
$ForumLang['UserProfileAvatarsAllowed'] = 'Допустимые типы файлов';
$ForumLang['UserProfileAttachb'] = 'Байт';
$ForumLang['UserProfileAttachkb'] = 'КБайт';
$ForumLang['UserProfileAttachmb'] = 'МБайт';
$ForumLang['UserProfileAvatarAllowedGabarits'] = 'Допустимые габариты изображения';
$ForumLang['UserProfileAvatarhWidth'] = 'ширина';
$ForumLang['UserProfileAvatarhHeight'] = 'высота';
$ForumLang['UserProfileAvatarAllowedSize'] = 'Допустимый размер картинки';
$ForumLang['UserProfileICQ'] = 'ICQ';
$ForumLang['UserProfilePhone'] = 'Домашний телефон';
$ForumLang['UserProfileMobile'] = 'Мобильный телефон';
$ForumLang['UserProfileNotAvail'] = 'Недоступно';
$ForumLang['UserProfileNoAvatar'] = 'Пользователь еще не загрузил аватар';
$ForumLang['UserProfileOwnerNoAvatar'] = 'Вы еще не загрузили аватар';
$ForumLang['UserProfileAllThemes'] = 'Все темы пользователя';
$ForumLang['UserProfileSubmit'] = 'Редактировать профиль';
$ForumLang['UserProfileReset'] = 'Очистить форму';
$ForumLang['UserProfileAllPosts'] = 'Все сообщения пользователя';
$ForumLang['UserProfileAdmMail'] = 'Разрешать администратору слать сообщения на почту';
$ForumLang['UserProfileOthMail'] = 'Разрешать пользователям слать сообщения на почту';
$ForumLang['UserProfileAdminMailProfile'] = 'Редактирование профиля на';
$ForumLang['UserProfileYourOwnProfile'] = 'ваш профиль';
$ForumLang['UserProfileUsers'] = 'Все пользователи';
$ForumLang['UserProfileNoThemes'] = 'Пользователь не оставил ни одной темы';
$ForumLang['UserProfileNoPosts'] = 'Пользователь не оставил ни одного сообщения';
$ForumLang['UserProfileDeleteUser'] = 'Удалить';
$ForumLang['UserProfileEditUser'] = 'Редактировать';
$ForumLang['UserProfileCaptchaTitle'] = 'Защитный код';
$ForumLang['UserProfileActions'] = array ();
$ForumLang['ProfileMail']['EditMailTheme'] = 'Редактирование профиля на';
$ForumLang['UserProfileActions']['main_page'] = 'просматривает главную страницу';
$ForumLang['UserProfileActions']['forum_page'] = 'просматривает темы форумов';
$ForumLang['UserProfileActions']['theme_page'] = 'просматривает сообщения темы';
$ForumLang['UserProfileActions']['add_theme'] = 'добавляет тему в форум';
$ForumLang['UserProfileActions']['add_post'] = 'добавляет пост в тему';
$ForumLang['UserProfileActions']['edit_post'] = 'редактирует пост';
$ForumLang['UserProfileActions']['user_list'] = 'просматривает список пользователей';
$ForumLang['UserProfileActions']['own_profile'] = 'редактирует профиль';
$ForumLang['UserProfileActions']['user_profile'] = 'просматривает профиль';
$ForumLang['UserProfileActions']['rss_page'] = 'подписывается на RSS';
$ForumLang['UserProfileErrors'] = array ();
$ForumLang['UserProfileErrors']['ErrorBlockTitle'] = 'Ошибки при редактировании профиля';
$ForumLang['UserProfileErrors']['EmptyLogin'] = 'Не заполнено поле логина';
$ForumLang['UserProfileErrors']['EmptyMail'] = 'Не заполнено поле почты';
$ForumLang['UserProfileErrors']['EmptyRepeatMail'] = 'Не заполнено поле повтора почты';
$ForumLang['UserProfileErrors']['EmptyDate'] = 'Не заполнено поле даты';
$ForumLang['UserProfileErrors']['BadMailLength'] = 'Длина электронной почты должна составлять не менее {min} и не более {max} символов';
$ForumLang['UserProfileErrors']['BadMailSymbols'] = 'Некорректный формат электронной почты';
$ForumLang['UserProfileErrors']['MailsNoEq'] = 'Ыыеденные почты не совпадают';
$ForumLang['UserProfileErrors']['MailExists'] = 'Пользователь с такой электронной почтой уже зарегистрирован';
$ForumLang['UserProfileErrors']['EmptyDate'] = 'Не заполнено поле даты рождения';
$ForumLang['UserProfileErrors']['WrongDateFormat'] = 'Дата должна быть в виде дд.мм.гггг';
$ForumLang['UserProfileErrors']['WrongAvatar'] = 'Загруженный вами в ачестве аватара не является изображением';
$ForumLang['UserProfileErrors']['WrongICQFormat'] = 'Формат ICQ не является правильным';
$ForumLang['UserProfileErrors']['WrongPhoneFormat'] = 'Формат домашнего телефона не является правильным';
$ForumLang['UserProfileErrors']['WrongMobileFormat'] = 'Формат мобильного телефона не является правильным';
$ForumLang['UserProfileErrors']['WrongSiteLength'] = 'Длина домашней страны должна быть не меньше {min}  не больше {max} символов';
$ForumLang['UserProfileErrors']['WrongSiteFormat'] = 'Некорректный домашней страницы';
$ForumLang['UserProfileErrors']['WrongCityLength'] = 'Длина названия города должна быть не меньше {min}  не больше {max} символов';
$ForumLang['UserProfileErrors']['WrongCityFormat'] = 'Не тот формат названия города';
$ForumLang['UserProfileErrors']['WrongPasswordLength'] = 'Длина пароля должна составлять не менее {min} и не больше {max} символов';
$ForumLang['UserProfileErrors']['WrongPasswordEqual'] = 'Введенные пароли не совпадают';
$ForumLang['UserProfileErrors']['WrongCountryLength'] = 'Длина названия страны должна быть не меньше {min}  не больше {max} символов';
$ForumLang['UserProfileErrors']['WrongCountryFormat'] = 'Не тот формат названия страны';
$ForumLang['UserProfileErrors']['WrongSloganLength'] = 'Количество символов в подиси не должно превышать {max}';
$ForumLang['UserProfileErrors']['WrongSloganRows'] = 'Количество строк в подиси не должно превышать {max}';
$ForumLang['UserProfileErrors']['WrongAvatar'] = 'Файл, загруженный вами в качестве аватара, не является картинкой';
$ForumLang['UserProfileErrors']['BadCaptcha'] = 'Вы ввели неправильно защитный код';
$ForumLang['UserProfileErrors']['EmptyCaptcha'] = 'Вы не ввели защитный код';
$ForumLang['UserProfileErrors']['CaptchaError'] = 'Ошибка защитного кода';
//Профиль пользователя - конец

?>